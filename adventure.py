...
class Character:
    def __init__(self, name, health, attack_power):
        ...
        # Set the character's current health to max
        self.health = health

        # Remembers the character's max health to reference later
        self.max_health = health

        # Sets character's attack power
    ...
    def attack(self, opponent, variance=0.2):
        """Randomizes damage based on attack power and a universal variance value.
        It then reduces the opponent's health by that value."""
    ...
    def heal(self, variance=0.3):
        """Heals a randomized value using a universal variance"""
        base_heal_value = 30
        ...
        print(
            f"{self.name} drinks a potion and recovers {recovery} health! Current health: {self.health}"
        )

    def use_special(self, opponent):
        """This is a generic function to be defined by each subclass."""
        return False
...
class Warrior(Character):
    ...
    def use_special(self, opponent):
        ...
        print(
            f"2. Reckless Swing (Deals 2.0x damage, take 15 recoil) {self.get_cooldown_text(2)}"
        )
        print("3. Cancel")
...
class Mage(Character):
    ...
    def use_special(self, opponent):
        print("\n--- Mage Abilities ---")
        print(
            f"1. Unstable Fireball (Wild damage variance: 0.5x to 2.0x) {self.get_cooldown_text(1)}"
        )
        print(
            f"2. Vampiric Drain (Deals damage, heals you for 100% of it) {self.get_cooldown_text(2)}"
        )
        print("3. Cancel")
        ...
        if choice == "1":
            ...
            damage = round(
                random.uniform(self.attack_power * 0.5, self.attack_power * 2.0)
            )
            opponent.health -= damage
...
class Thief(Character):
    ...
    def use_special(self, opponent):
        print("\n--- Thief Abilities ---")
        print(
            f"1. Flurry of Knives (Hits twice for 0.6x damage each) {self.get_cooldown_text(1)}"
        )
        print(
            f"2. Shadow Strike (50% chance to deal 2.5x damage, or miss) {self.get_cooldown_text(2)}"
        )
        print("3. Cancel")
        ...
        if choice == "1":
            ...
            if random.random() > 0.5:
                ...
                opponent.health -= damage
                print(
                    f"A critical hit! {self.name} strikes from the shadows for {damage} damage!"
                )
            else:
...
class Paladin(Character):
    ...
    def use_special(self, opponent):
        print("\n--- Paladin Abilities ---")
        print(
            f"1. Holy Smite (Deals damage and heals you for 50% of it) {self.get_cooldown_text(1)}"
        )
        print(
            f"2. Lay on Hands (Heals you for a massive 60 HP) {self.get_cooldown_text(2)}"
        )
        print("3. Cancel")
        ...
        if choice == "1":
            ...
            print(
                f"{self.name} strikes for {damage} damage and is healed by the light for {heal_amount} HP!"
            )
            self.cooldowns[1] = 3
            return True
            ...
            self.health = min(self.max_health, self.health + heal_amount)
            print(
                f"A blinding light surrounds {self.name}, restoring {heal_amount} health!"
            )
            self.cooldowns[2] = 4
            return True
        else:
...
class EvilWizard(Character):
    ...
    # Heals the Evil Wizard for a random value between 5 and 10 without overhealing
    def regenerate(self):
        regen_value = random.randint(5, 10)
        self.health = min(self.max_health, self.health + regen_value)
        print(
            f"{self.name} passively regenerates {regen_value} health! Current health: {self.health}"
        )
    ...
    def take_turn(self, opponent):
        ...
        if roll < 0.05:
            ...
            opponent.health -= damage
            print(
                f"A massive beam of void energy hits {opponent.name} for {damage} damage!"
            )
        elif roll < 0.20:
            ...
            print(
                f"The wizard drains {damage} health from {opponent.name} and heals himself for {heal}!"
            )
        else:
...
def battle(player, wizard):
    while wizard.health > 0 and player.health > 0:
        ...
        if choice == "1":
            ...
            print(
                f"Enemy {wizard.name}'s Health: {max(0, wizard.health)}/{wizard.max_health}"
            )
            continue
        else:
